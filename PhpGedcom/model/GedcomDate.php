 ...
 * @package PhpGedView
 * @author Greg Roach
 * @version $Id: class_date.php 6652 2010-01-01 20:32:39Z yalnifj $
 *
...
class JulianDate extends CalendarDate {
	...
	function FormatLongYear() {
		global $pgv_lang;
		if ($this->y<0)
			if (isset($pgv_lang['b.c.'])) return (-$this->y).$pgv_lang['b.c.'];
			else return (-$this->y).'b.c.';
		else
...
class JewishDate extends CalendarDate {
	...
	function FormatLongMonth() {
		...
		if ($mon=='adr' && $this->IsLeapYear())
			$mon.='_leap_year';
		if (isset($pgv_lang[$mon])) return $pgv_lang[$mon];
		else return $mon;
	}
