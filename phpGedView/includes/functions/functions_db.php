...
function search_indis_custom($orderList, $join, $where, $order) {
	global $TBLPREFIX;

	// postgreSQL insists that any column mentioned in the ORDER BY clause also appear in the SELECT list.  Other database types do this automatically.
	$sql = "SELECT DISTINCT 'INDI' AS type, i_id AS xref, i_file AS ged_id, i_gedcom AS gedrec, i_isdead, i_sex";
	if (!empty($orderList)) {
		$sql .= ', '.implode(',', $orderList);
	}
	$sql .= " FROM {$TBLPREFIX}individuals ".implode(' ', $join).' WHERE '.implode(' AND ', $where);
	if (!empty($order)) {
...
function search_fams_custom($orderList, $join, $where, $order) {
	global $TBLPREFIX;

	// postgreSQL insists that any column mentioned in the ORDER BY clause also appear in the SELECT list.  Other database types do this automatically.
	$sql = "SELECT DISTINCT 'FAM' AS type, f_id AS xref, f_file AS ged_id, f_gedcom AS gedrec, f_husb, f_wife, f_chil, f_numchil";
	if (!empty($orderList)) {
		$sql .= ', '.implode(',', $orderList);
	}
	$sql .= " FROM {$TBLPREFIX}families ".implode(' ', $join).' WHERE '.implode(' AND ', $where);
	if (!empty($order)) {
		$sql .= ' ORDER BY '.implode(',', $order);
	}
