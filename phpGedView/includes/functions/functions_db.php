...
//-- gets the sorted list of xref's in the gedcom
function get_sorted_xref_list($type, $ged_id=PGV_GED_ID) {
	global $TBLPREFIX;

	switch ($type) {
	case "INDI":
		$statement = PGV_DB::prepare("SELECT i_id FROM {$TBLPREFIX}individuals WHERE i_file=?")
				->execute(array($ged_id));
		break;
	case "FAM":
		$statement = PGV_DB::prepare("SELECT f_id FROM {$TBLPREFIX}families WHERE f_file=?")
				->execute(array($ged_id));
		break;
	case "SOUR":
		$statement = PGV_DB::prepare("SELECT s_id FROM {$TBLPREFIX}sources WHERE s_file=?")
				->execute(array($ged_id));
		break;
	case "OBJE":
		$statement = PGV_DB::prepare("SELECT m_media FROM {$TBLPREFIX}media WHERE m_gedfile=?")
				->execute(array($ged_id));
		break;
	default:
		$statement = PGV_DB::prepare("SELECT o_id FROM {$TBLPREFIX}other WHERE o_file=? AND o_type=?")
...
function get_next_xref($pid, $ged_id=PGV_GED_ID) {
	...
	if (array_key_exists($x+1,$templist))
		return $templist[$x+1];
	else
		return "";
}
...
/* Generate a list of column names mentioned in the ORDER BY clause that aren't also in the SELECT list
 *		This list is used to extend the SELECT list so that certain database systems don't complain.
 *
 *		Currently, some database systems insist that the column list of SELECT DISTINCT also include the column names
 *		mentioned in an associated ORDER BY clause.  There are no complaints when there is no ORDER BY clause.
 *
 *		We will make sure that only those ORDER BY columns not already mentioned in the SELECT clause will be added.  This
 *		will avoid possible problems when the extended SELECT list mentions the same column more than once.
 */
function examineOrderBy($selectList, $orderList) {
	if (empty($orderList)) return '';		// Nothing to do here
	// Prepare the list of current SELECT column names for easier searching
	$selectList = str_replace(array(' AS ', ' as  ', ','),' ',$selectList);		// Get rid of "as" and commas
	$selectList = ' '.$selectList.' ';											// and tack on some spaces as guards
	if (is_string($orderList)) {
		$orderList = explode(',', $orderList);		// This should be an array
	}

	$result = '';
	foreach($orderList as $columnName) {
		$columnName = trim($columnName);		// Make sure this doesn't start with a space
...
function search_indis_custom($join, $where, $order) {
	...
	$selectList = "'INDI' AS type, i_id AS xref, i_file AS ged_id, i_gedcom AS gedrec, i_isdead, i_sex";
	$moreSelects = examineOrderBy($selectList, $order);		// Generate a possible extension to the SELECT list

	$sql = "SELECT DISTINCT $selectList";
	$sql .= "$moreSelects FROM {$TBLPREFIX}individuals ".implode(' ', $join).' WHERE '.implode(' AND ', $where);
	if (!empty($order)) {
...
function search_fams_custom($join, $where, $order) {
	...
	$selectList = "'FAM' AS type, f_id AS xref, f_file AS ged_id, f_gedcom AS gedrec, f_husb, f_wife, f_chil, f_numchil";
	$moreSelects = examineOrderBy($selectList, $order);		// Generate a possible extension to the SELECT list

	$sql = "SELECT DISTINCT $selectList";
	$sql .= "$moreSelects FROM {$TBLPREFIX}families ".implode(' ', $join).' WHERE '.implode(' AND ', $where);
	if (!empty($order)) {
		$sql .= ' ORDER BY '.implode(',', $order);
	}
