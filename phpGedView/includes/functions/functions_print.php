...
function print_pedigree_person($pid, $style=1, $show_famlink=true, $count=0, $personcount="1") {
	...
	// Calculate the desired position of the popup menu
	if (isset($boxPosn)) {
		$menuLeftPosn = $boxPosn + $bwidth + $brborder;
	echo "\n<!-- menuLeftPosn: {$menuLeftPosn}; boxPosn: {$boxPosn}; bwidth: {$bwidth}; brborder: {$brborder}. -->\n";
	} else $menuLeftPosn = 0;
	...
	if ($person->canDisplayName()) {
		if ($show_famlink && (empty($SEARCH_SPIDER))) {
			...
			if ($LINK_ICONS!="disabled") {
				...
				$icons .= "><img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["pedigree"]["small"]."\" width=\"25\" border=\"0\" vspace=\"0\" hspace=\"0\" alt=\"\" title=\"\" /></a>";
			}
		}
		else {
			if ($style==1) {
...
function print_text($help, $level=0, $noprint=0){
	...
	$pos1 = 0;
	$pos2 = 0;
	$ct = preg_match_all('/#([a-zA-Z0-9:_.\-\[\]]+)#/', $sentence, $match, PREG_SET_ORDER);
	for($i=0; $i<$ct; $i++) {
		$value = "";
